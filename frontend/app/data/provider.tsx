...
const mapFeeStatus = (status: string): Student["status"] => {
...
const mapStudent = (student: any): Student => {
  ...
  return {
    ...
    paidAmount,
    dueAmount,
    status: mapFeeStatus(student.feeStatus || (dueAmount === 0 && paidAmount > 0 ? "PAID" : paidAmount > 0 ? "PARTIAL" : "PENDING")),
    discount: "None",
    feeCategory: "Tuition",
    joinDate: student.createdAt?.slice(0, 10) || new Date().toISOString().slice(0, 10),
    dueDate: student.dueDate?.slice(0, 10) || "",
  }
}
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!token) {
      console.log("[DataProvider] No token available, skipping data load")
      return
    }
    ...
    try {
      ...
      console.log("[DataProvider] User role:", user?.role)
      console.log("[DataProvider] User profile:", user?.profile ? "exists" : "missing")
      
      const studentUrl = user?.role === "STUDENT" && user.profile?.id
        ? `/students/${user.profile.id}`
        : "/students?limit=100&page=1"
      
      console.log("[DataProvider] Fetching students from:", studentUrl)
      
      const [studentResponse, paymentResponse, receiptResponse, feeResponse] = await Promise.all([
        apiRequest<any>(studentUrl).catch(err => {
          console.error("[DataProvider] Students request failed:", err.message)
          throw err
        }),
        apiRequest<any>("/payments?limit=100").catch(err => {
          console.error("[DataProvider] Payments request failed:", err.message)
          throw err
        }),
        apiRequest<any>("/receipts").catch(err => {
          console.error("[DataProvider] Receipts request failed:", err.message)
          throw err
        }),
        apiRequest<any>("/fee-structures?limit=100").catch(err => {
          console.error("[DataProvider] Fee structures request failed:", err.message)
          throw err
        }),
      ])

      console.log("[DataProvider] Student response:", studentResponse)
      const studentRows = Array.isArray(studentResponse.data) ? studentResponse.data : [studentResponse.data]
      console.log("[DataProvider] Mapped students:", studentRows.length)
      ...
      console.error("[DataProvider] Error loading data:", errorMsg)
      setError(errorMsg)
    } finally {
      setLoading(false)
    }
  ...
  const updateStudent = async (s: Student) => {
    const response = await apiRequest<{ data: any }>(`/students/${s.id}`, {
      ...
      body: JSON.stringify({
        ...
        feeStatus: s.status.toUpperCase(),
      }),
    })
