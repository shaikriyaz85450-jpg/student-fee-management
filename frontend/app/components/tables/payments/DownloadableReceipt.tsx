...
export const DownloadableReceipts: React.FC<DownloadableReceiptsProps> = ({
  ...
  const storeReceipts = payments.map(p => ({
    id: p.id,
  ...
  const displayReceipts = receipts ?? storeReceipts
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card">
      ...
      <div className="border-t border-border bg-muted/20 px-6 py-4">
        <p className="text-xs text-muted-foreground">
          Showing {displayReceipts.length} receipt{displayReceipts.length !== 1 ? "s" : ""}. All receipts are sent to your email as well.
        </p>
      </div>
    </div>
  )
