...
import { Bell, CreditCard, Download, AlertTriangle, IndianRupee, Clock, CheckCircle } from "lucide-react"
...
export default function PendingFeesPage() {
  const { students, pendingFees, updateStudent, refresh } = useData()
  ...
  const toggleStatus = async (p: any) => {
    ...
    if (!student) return
    try {
      await updateStudent({ ...student, status: "Paid" })
      await refresh()
