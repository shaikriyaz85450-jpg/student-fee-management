...
import { Download, Edit2, Receipt, Search, CheckCircle } from "lucide-react"
...
export default function StudentRecordsPage() {
  ...
  const { students, addStudent, updateStudent, refresh } = useData()
  ...
  const toggleStatus = async (s: Student) => {
    const newStatus = s.status === "Paid" ? "Pending" : "Paid"
    try {
      await updateStudent({ ...s, status: newStatus as any })
      await refresh()
  ...
  return (
    <div className="space-y-4">
      <Topbar title="Student records" subtitle={`${filtered.length} students`}>
          ...
          <EditStudentDialog student={selectedStudent} open={editOpen} onOpenChange={setEditOpen} onSuccess={() => {}} />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40">
                  {["Student", "Roll No", "Class", "Total Fee", "Paid", "Due", "Due Date", "Progress", "Status", "Actions"].map((h) => (
                  ...
                  return (
                    <tr key={s.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-4 py-3 text-muted-foreground text-xs whitespace-nowrap">
                        {s.dueDate ? new Date(s.dueDate).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' }) : "-"}
                      </td>
                       ...
                       <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            className={cn("h-7 w-7", s.status === "Paid" ? "text-emerald-600" : "text-muted-foreground")} 
                            onClick={() => toggleStatus(s)}
                            title={s.status === "Paid" ? "Mark as Pending" : "Mark as Paid"}
                          >
